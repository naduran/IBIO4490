...
resume = './32VGG/checkpoint.pth.tar'
...
def imshow_label(label_show,tag,alpha=None):
    import matplotlib
    cmap = plt.cm.jet
...
def run_fromfile(img_file,tag):
    ...
    with torch.no_grad():
        ...
        imshow_label(lbl_pred[0], tag, alpha=0.5)
        plt.savefig(f'./{tag}/Predicted_{name}.png')
        plt.show()
        

        imshow_label(lbl_pred[0],tag)
        plt.savefig(f'./{tag}/Segmentation_{name}.png')
        plt.show()

img_file = 'imgs/demo.jpg'
run_fromfile(img_file,tag)
