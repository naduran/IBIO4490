...
def train(model):
    ...
    for i in range(epochs):
        ...
        for j in range(0,x_train.shape[0], batch_size):
            ...
            model.compute_gradient(_x_train[0:round(x_train.shape[0]/2)], out, _y_train[0:round(y_train.shape[0]/2)])
        out = model.forward(x_train[round(x_train.shape[0]/2):x_train.shape[0]])                
        loss_test = model.compute_loss(out,y_train[round(y_train.shape[0]/2):y_train.shape[0]])
    ...
    pmodel = {'W':model.W,'b':model.b}
    with open('model40.pickle','wb') as f:
...
def plot(ep,loss_train,loss_test): # Add arguments
    ...
    pdf.savefig("40kepoch_100batch_val.pdf")
    # CODE HERE
...
def test(model):
    ...
    y= np.array(y_test,dtype=np.int)
    p, r, _ = precision_recall_curve(y,prediction)
    
    c_pr=plt.figure()
    plt.plot(r,p)
    plt.xlabel('Recall')
    plt.ylabel('Precision')
    c_pr.savefig("P_R.pdf")
