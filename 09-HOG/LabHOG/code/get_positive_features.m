...
features_pos=zeros(num_images,dim); 
im1=imresize(im1,[feature_params.template_size,feature_params.template_size])
features_pos(1,:) =reshape(vl_hog(im2single(im1), cellSize),[1,dim]);
for i=2:num_images
    im2=imread(fullfile( 'data','caltech_faces','Caltech_CropFaces',image_files(i).name));
    im2=imresize(im2,[feature_params.template_size,feature_params.template_size])
